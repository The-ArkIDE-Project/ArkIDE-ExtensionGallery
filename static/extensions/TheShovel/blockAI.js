...
(function () {
  ...
  // "Why not fetch this?"
  // I cant because i have to set the flag color with the variable above. Thats literally the only reason lmao
  ...
  // On TurboWarp you cant fetch the icon from a link so we have to include it here like this
  const icon =
  ...
  /*
  I am providing my API key for free. I would hide it somehow but the TurboWarp/Penguinmod extension apis dont have some kind of "secrets" system!!!
  I encoded it like this to avoid bots taking it. The bare minimum. Maybe i should build some kind of proxy for this...
  */
  ...
  function setBuddyState(state) {
    buddyBox.style.backgroundImage = `url(${assetsPath}Images/Expressions/${buddyStyle}/${state}.png)`;
  }
  ...
  if (localStorage.getItem("blockaigeminiext.settings") == null) {
    ...
    buddyStyle = savedSettings.buddyStyle;
  ...
  async function fetchtrainingdata() {
    const response = await fetch(`${assetsPath}trainingdata`);
    if (!response.ok) throw new Error("Error fetching training data");
    modelInstructions = await response.text();
  ...
  function createWindow() {
    ...
    mainWindow.appendChild(minimizeButton);
    addElementClickEffect(minimizeButton);
    let heightBeforeMin;
    minimizeButton.onclick = () => {
      mainWindow.style.transition = "height 0.3s ease, min-height 0.3s ease";
      ...
      if (isMinimized) {
        ...
        setTimeout(() => {
          dragCorner.style.opacity = "1";
          buddyBox.style.opacity = "1";
          responseArea.style.opacity = "1";
          inputContainer.style.opacity = "1";

          mainWindow.style.resize = "both";
          setTimeout(() => {
        ...
        setTimeout(() => {
          dragCorner.style.visibility = "hidden";
          mainWindow.style.height = "30px";
          mainWindow.style.minHeight = "30px";
          setTimeout(() => {
            mainWindow.style.transition = "";
          }, 300);
        }, 250);
    ...
    responseArea.style.cssText = `
      ...
      background-image: radial-gradient(rgba(128, 128, 128, 0.16) 1px, transparent 0px);
    ...
    buddyBox = document.createElement("div");
    buddyBox.style.cssText = `
      transition: all 0.2s ease;
      position: absolute;
      width: 107px;
      height: 133px;
  ...
  async function testAllModels() {
    if (!apiKey) {
      alert("Please set your API key first before testing models.");
      return;
    }

    const testButton = document.getElementById("test-models-button");
    const originalText = testButton.textContent;
    testButton.textContent = "Fetching available models...";
    testButton.disabled = true;

    let availableModels = [];

    // First, get the list of available models from Google API
    try {
      ...
      if (modelsResponse.ok) {
        ...
        console.warn("Failed to fetch available models list");
      }
    } catch (error) {
      ...
      contents: [
        {
          parts: [
            {
              text: "Hello, respond with just 'OK' to confirm you're working.",
            },
          ],
        },
      ],
    ...
    for (let i = 0; i < testedModels.length; i++) {
      const model = testedModels[i];
      testButton.textContent = `Testing ${i + 1}/${testedModels.length}: ${model}`;

      const isInOfficialList =
      ...
      try {
        ...
        if (response.ok && result.candidates && result.candidates.length > 0) {
          const status = isInOfficialList
            ? "Working ✅"
            : "Working ✅ (Not in official list)";
          results.push({
            model,
            status,
            error: null,
            inOfficialList: isInOfficialList,
          });
        } else {
          const error = result.error?.message || "Unknown error";
          const status = isInOfficialList
            ? "Failed ❌"
            : "Failed ❌ (Not in official list)";
          results.push({
            model,
            status,
            error,
        ...
        let errorMessage = "Connection timeout or network error";
    ...
    const failedModels = results.filter((r) => r.status.includes("Failed ❌"));

    const workingInOfficial = workingModels.filter((r) => r.inOfficialList);
    const workingNotInOfficial = workingModels.filter((r) => !r.inOfficialList);
    const failedInOfficial = failedModels.filter((r) => r.inOfficialList);
    const failedNotInOfficial = failedModels.filter((r) => !r.inOfficialList);

    if (workingInOfficial.length > 0) {
      resultText += "✅ WORKING MODELS (in official list):\n";
      workingInOfficial.forEach(({ model }) => {
        resultText += `  • ${model}\n`;
      });
      resultText += "\n";
    }

    if (workingNotInOfficial.length > 0) {
      resultText += "✅ WORKING MODELS (not in official list):\n";
      workingNotInOfficial.forEach(({ model }) => {
        resultText += `  • ${model}\n`;
      });
      resultText += "\n";
    }

    if (failedInOfficial.length > 0) {
      resultText += "❌ FAILED MODELS (in official list):\n";
      failedInOfficial.forEach(({ model, error }) => {
        resultText += `  • ${model}\n`;
        if (error) {
    ...
    if (failedNotInOfficial.length > 0) {
      resultText += "❌ FAILED MODELS (not in official list):\n";
      failedNotInOfficial.forEach(({ model, error }) => {
        resultText += `  • ${model}\n`;
        if (error) {
          resultText += `    Error: ${error}\n`;
        }
    ...
    resultText += `SUMMARY: ${workingModels.length}/${totalModels} models working`;
    if (workingNotInOfficial.length > 0) {
      resultText += ` (${workingNotInOfficial.length} working but not in official list)`;
    }
  ...
  function openSettingsWindow(shiftPressed = false) {
    ...
    let buddyStyleLabel = document.createElement("label");
    buddyStyleLabel.textContent = "Buddy Style:";
    buddyStyleLabel.style.cssText = basicLabelStyle;
    settingsWindow.appendChild(buddyStyleLabel);

    let buddyStyleSelect = document.createElement("select");
    buddyStyleSelect.style.cssText = basicInputStyle;
    const buddyStylesList = [
      "full-color",
      "white-outline",
      "black-outline",
      "keith",
    ...
    buddyStylesList.forEach((style) => {
      ...
      option.value = style;
      option.textContent = style;
      buddyStyleSelect.appendChild(option);
    });
    ...
    if (shiftPressed) {
      ...
      testModelsButton.id = "test-models-button";
      testModelsButton.textContent = "Test All Models";
      testModelsButton.style.cssText = basicButtonStyle;
      settingsWindow.appendChild(testModelsButton);
      addElementClickEffect(testModelsButton);
      testModelsButton.onclick = testAllModels;
      makeDivider();
    }
    ...
    saveButton.onclick = () => {
      ...
      buddyStyle = buddyStyleSelect.value;
      setBuddyState("idle");
      fixBlockStyleSize();
      let settingsToSave = {
        style: selectedStyle,
  ...
  async function getGeminiResponse(prompt) {
    ...
    if (!apiKey) {
      responseArea.innerHTML =
        '<div class="fade-in">API Key not set. Set your Key in the settings.</div>';
      setTimeout(() => {
        const newElement = responseArea.lastElementChild;
        if (newElement) newElement.classList.add("loaded");
      }, 10);
      return;
    }
    ...
    if (sendContextCheck.checked) {
      ...
      try {
        ...
        responseArea.innerHTML += `<div class="fade-in"><b>${aiName}:</b> Error obtaining script data. Could not include it in the request.</div>`;
        setTimeout(() => {
          const newElement = responseArea.lastElementChild;
          if (newElement) newElement.classList.add("loaded");
        }, 10);
    ...
    let uploadResult;
    const fullPrompt =
      "These are guied for how to act:" +
      "\n" +
      modelInstructions +
      "\n" +
      "This is your conversation history, continue this: " +
      "\n" +
      history +
      "\n" +
      "User:" +
    ...
    try {
      responseArea.innerHTML +=
        "<div class='fade-in'><b>You:</b> " +
        promptInput.value +
        "</div><div id='generating-temp' class='fade-in'></div>";
      setTimeout(() => {
        const elements = responseArea.querySelectorAll(".fade-in:not(.loaded)");
        elements.forEach((el) => el.classList.add("loaded"));
      }, 10);

      if (sendContextCheck.checked) {
        const generatingTemp = document.getElementById("generating-temp");
        generatingTemp.classList.remove("loaded");
        generatingTemp.textContent = "Reading sprite...";
        setTimeout(() => {
          generatingTemp.classList.add("loaded");
        }, 10);
      ...
      generationConfig = {
        ...
        topK: 40, //no idea
        topP: 0.95, //no idea again lmao
        maxOutputTokens: 2048, //keeps responses small
      };
      ...
      const generatingTemp = document.getElementById("generating-temp");
      generatingTemp.classList.remove("loaded");
      generatingTemp.textContent = "Generating response...";
      setTimeout(() => {
        generatingTemp.classList.add("loaded");
      }, 10);
      ...
      const tryModelWithTimeout = async (model, timeoutMs = 8000) => {
        const controller = new AbortController();
      ...
      // Check if current model has already failed in this session
      if (!sessionFailedModels.includes(geminiVersion)) {
        const geminiUrl =
          `https://generativelanguage.googleapis.com/v1beta/models/` +
          geminiVersion +
          `:generateContent?key=${apiKey}`;
        ...
        try {
          ...
          if (
            ...
            successfulModel = geminiVersion;
          } else {
            throw new Error("Current model failed");
          }
        } catch (error) {
          console.log(`Model ${geminiVersion} failed:`, error);
          sessionFailedModels.push(geminiVersion);
          triedModels.push(geminiVersion);
        }
      } else {
        console.log(
          `Model ${geminiVersion} previously failed, skipping to fallbacks`,
        );
      }

      // If current model failed or was skipped, try fallback models
      if (!successfulModel) {
        const fallbackModels = testedModels.filter(
          (model) =>
            model !== geminiVersion && !sessionFailedModels.includes(model),
        );

        const generatingTemp = document.getElementById("generating-temp");
        if (generatingTemp && fallbackModels.length > 0) {
          generatingTemp.classList.remove("loaded");
          generatingTemp.textContent = "Model failed, trying another...";
          setTimeout(() => {
            generatingTemp.classList.add("loaded");
          }, 10);
        }
        ...
        for (const currentModel of fallbackModels) {
          ...
          if (generatingTemp) {
            const modelDisplayName = currentModel
              ...
              .replace(/preview-/i, "");
            generatingTemp.classList.remove("loaded");
            generatingTemp.textContent = `Trying ${modelDisplayName}...`;
            setTimeout(() => {
              generatingTemp.classList.add("loaded");
            }, 10);
          }

          const attempt = await tryModelWithTimeout(currentModel);

          if (attempt.success) {
            successfulModel = currentModel;
            result = attempt.result;
            apiResponse = attempt.response;
            break;
          } else {
            console.log(
            ...
            sessionFailedModels.push(currentModel);
      ...
      if (
        ...
        const newlyFailedModels = triedModels.filter(
          (model) => !shownFailureMessages.includes(model),
        );
        if (newlyFailedModels.length > 0 && successfulModel !== geminiVersion) {
          const failedModel = newlyFailedModels[newlyFailedModels.length - 1];
          responseArea.innerHTML += `<div class="fade-in" style="color: #666; font-size: 0.9em; margin-bottom: 10px;"><i>Note: Using ${successfulModel}</i></div>`;
          setTimeout(() => {
            const newElement = responseArea.lastElementChild;
            if (newElement) newElement.classList.add("loaded");
          }, 10);
          shownFailureMessages.push(...newlyFailedModels);
        }
        titleBar.textContent =
          aiName +
          " Chat - " +
          successfulModel.replace(/gemini-/i, "").replace(/preview-/i, "") +
          " (" +
        ...
        const scratchCodeRegex = /```scratch([\s\S]*?)```/g;
        let match = null;
        ...
        intermingledResponseText = responseText;
        const matches = [];
        ...
        for (let i = matches.length - 1; i >= 0; i--) {
          ...
          const scratchCode = currentMatch.code;
          console.log("Original scratchCode:", scratchCode);
          let modifiedScratchCode = scratchCode
            .replace(
              /<\(\s*([\s\S]*?)\s*\)\s*(>=|<=|=)\s*\(\s*([\s\S]*?)\s*\)>/g,
              (m, variable, operator, value) =>
                operator === ">="
          ...
          console.log("Regex scratchCode:", modifiedScratchCode);

          const pre = document.createElement("pre");
          pre.innerHTML = modifiedScratchCode;

          // Replace the original markdown block with the HTML <pre> tag
          intermingledResponseText =
            intermingledResponseText.substring(0, currentMatch.index) +
            pre.outerHTML +
            intermingledResponseText.substring(
              currentMatch.index + currentMatch.fullMatch.length,
            );
        }
        intermingledResponseText = intermingledResponseText.replace(
          /([.?!])\\\\s*(?=[A-Z])/g,
          "$1<br>",
        );
        responseArea.innerHTML +=
          ...
          aiName +
          `:</b> ${intermingledResponseText.replace(/([.?!])\\\\s*(?=[A-Z])/g, "$1<br>")}</div>`;
        history.push(
          "<b>User:</b> " +
            prompt +
            "/n" +
        ...
        if (newResponseDiv) {
          setTimeout(() => {
            ...
            if (scratchblocks && scratchblocks.renderMatching) {
              ...
              if (newResponseDiv.querySelector(".sb3-obsolete")) {
                setBuddyState("mistake");
                responseArea.innerHTML += `<div class="fade-in" style="color: #666; font-size: 0.9em; margin-bottom: 10px;"><i>The output above has some errors! Oops...</i><button id="report-issue" style="margin-left: 10px; background-color: #ed4242; color: white; padding: 4px; text-align: center; text-decoration: none; display: inline-block; font-size: 12px; cursor: pointer; border-radius: 25px; border-style: solid; border-width: 1px; border-color: #d23636;">Report bug</button></div>`;
                setTimeout(() => {
                  const newElement = responseArea.lastElementChild;
                  if (newElement) {
                    newElement.classList.add("loaded");
                    const reportButton =
                      newElement.querySelector("#report-issue");
                    if (reportButton) {
                      reportButton.addEventListener("click", () => {
                        const randomString = Math.random()
                          ...
                          .substring(2, 10);
                        window.open(
                          `https://github.com/TheShovel/block-ai/issues/new?title=Scratchblocks%20syntax%20error%20(${randomString})&body=` +
                            "This is the result that causes the error:%0A```%0A" +
                            encodeURIComponent(
                              intermingledResponseText.replace(
                                /<\/?[^>]+(>|$)/g,
                                "",
                              ),
                            ) +
                            "%0A```" +
                            `%0AModel: ${successfulModel}%0A` +
                            "%0A(This is an automatically generated issue. If you are the one submitting it, you can just press the create/submit button. Thank you!)",
                          "_blank",
                        );
                      });
                    }
        ...
        if (sendContextCheck.checked && uploadResult && uploadResult.file) {
          fetch(
            `https://generativelanguage.googleapis.com/v1beta/${uploadResult.file.name}?key=${apiKey}`,
            {
              method: "DELETE",
            },
          ).catch((err) => console.log("Failed to delete sprite file:", err));
        }
      } else {
        let errorMessage = `All models failed. Tried: ${triedModels.join(", ")}.`;

        if (result && result.error && result.error.code === 429) {
          responseArea.innerHTML = `<div class="fade-in">All available models are rate-limited! Please provide your own API key.<br><a href="https://ai.google.dev/gemini-api/docs/api-key" target="_blank">https://ai.google.dev/gemini-api/docs/api-key</a></div>`;
          setTimeout(() => {
            const newElement = responseArea.lastElementChild;
            if (newElement) newElement.classList.add("loaded");
          }, 10);
          ...
          errorMessage = `All models blocked the prompt: ${result.promptFeedback.blockReason}.`;
          responseArea.innerHTML = `<div class="fade-in">${errorMessage}</div>`;
        } else {
          responseArea.innerHTML = `<div class="fade-in">${errorMessage}</div>`;
        }
      ...
      const errorMsg = `Error fetching response from Gemini API: ${apiError.message}`;
      responseArea.innerHTML = `<div class="fade-in">${errorMsg}</div>`;
      setTimeout(() => {
        const newElement = responseArea.lastElementChild;
        if (newElement) newElement.classList.add("loaded");
      }, 10);
